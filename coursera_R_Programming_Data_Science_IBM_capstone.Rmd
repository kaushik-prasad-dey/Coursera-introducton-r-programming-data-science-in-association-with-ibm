---
title: "Coursera Introduction to R Programming for Data Science in association with IBM"
author: "Kaushik Dey"
date: "8/28/2022"
...
```{r required library loaded}
library(tidyverse)
library(httr)
library(rvest)
```
## TASK 1: Get a COVID-19 pandemic Wiki page using HTTP request.
```{r get a covid-19 pandemic wiki page using HTTP request}
...
```{r Extract COVID-19 testing data table from the wiki HTML page}
...
```{r Get the first table in the HTML root node using html_node function, echo=TRUE}
...
```{r Pre-process and export the extracted data frame}
...
```
```{print the new_covid_data_frame}
## # A tibble: 6 x 7
...
```
```{print covid_data_frame_csv file}
...
## [1] "character"
```
```{Convert the country column into character}
...
```
```{print covid_data_frame_csv with matched_country}
...
```{Use if-else statement for covid dataframe}
